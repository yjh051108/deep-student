...
export const MemoryView: React.FC<MemoryViewProps> = ({
  ...
  const visibleMemoryFolderPaths = useMemo(() => {
    ...
    if (memoryScopeFilter === 'global') return ['全局'];
    return ['全局', ...topicMemoryRoots(topicGroupId, topicGroupName)];
  ...
  }, [config?.memoryRootFolderId, memoryScopeContext, memoryScopeFilter, visibleMemoryFolderPaths, hasTopicMemoryScope, t]);
                ...
                onClick={() => {
                  if (
                    ...
                    && !unifiedConfirm(t(
                      ...
                      '全部管理会显示所有课题的记忆，仅用于人工整理；Agent 默认仍只能看到当前课题 + 全局。确定切换吗？'
                    ))
                  ) {
                    return;
                  }
