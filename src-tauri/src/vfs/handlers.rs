...
use crate::vfs::repos::{
    ...
    VfsIndexStateRepo, VfsNoteRepo, VfsResourceRepo, VfsTextbookRepo, VfsTranslationRepo,
    INDEX_STATE_DISABLED, INDEX_STATE_PENDING,
};
use crate::vfs::types::*;
    ...
    if let Ok(mut stmt) = conn.prepare(
        ...
        if let Ok(rows) = stmt.query_map(
            ...
            for row in rows.flatten() {
                ...
                targets.push((
                    normalize_dstu_watch_path(Some(path), &item_id),
                    item_id,
                    item_type,
                ));
            }
...
use crate::vfs::indexing::{
    VfsEmbeddingStats, VfsIndexingService, VfsSearchParams, VfsSearchResult, VfsSearchService,
};
...
pub async fn vfs_get_multimodal_index_capability(
    ...
    let config_result = match llm_manager.get_vl_embedding_model_config().await {
        Ok(config) => Ok(config),
        Err(primary_err) => match resolve_bound_multimodal_embedding_model_id(&database, &vfs_db) {
            Ok(Some(model_config_id)) => {
                database
                    .save_setting(
                        "embedding.default_multimodal_model_config_id",
...
mod tests {
    ...
    fn index_status_row_parser_accepts_legacy_dimension_shapes() {
        ...
        let values = conn
            .query_row("SELECT NULL, 1024, 768.0, '512', ''", [], |row| {
                Ok((
