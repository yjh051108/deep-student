...
describe('chat mindmap embed scroll contract', () => {
  it('does not let chat-inline mindmaps consume wheel scrolling or swap from badge to card', () => {
    ...
    expect(embedSource).toContain('return height;');
    expect(rendererSource).not.toContain('<MindmapCitationBadge');
  });

  it('keeps inline and preview mindmap embeds from growing after load', () => {
    const embedSource = readFileSync(
      resolve(process.cwd(), 'src/features/mindmap/components/mindmap/MindMapEmbed.tsx'),
      'utf-8'
    );
    const previewRouterSource = readFileSync(
      resolve(process.cwd(), 'src/features/learning-hub/PreviewRouter.tsx'),
      'utf-8'
    );

    expect(embedSource).toContain('if (!expandLargeMaps) {\n      return height;\n    }');
    expect(previewRouterSource).toContain('expandLargeMaps={false}');
  });
});
