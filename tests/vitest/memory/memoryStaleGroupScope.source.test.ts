...
describe('memory and resource scope stale group contract', () => {
  ...
  it('resolves memory tool topic scope from active database state, not raw context hints', () => {
    ...
    expect(scopeHelper.indexOf('return None;')).toBeLessThan(
      scopeHelper.indexOf('return crate::memory::topic_memory_root(')
    );
    ...
    expect(visibleHelper).toContain('Self::effective_topic_scope(ctx)');
